...
// Folders re-exported from a layer barrel: `export * from './<slug>';`
...
if (exportedFolders.size === 0) {
  errors.push(
    'No `export * from` folders found in the primitives/components barrels — the guard cannot run',
  );
}
...
const allSpecimensBlock = specimenBarrel.match(/allSpecimens[^=]*=\s*\[([\s\S]*?)\]/)?.[1] ?? '';
for (const [folder, files] of specimenFiles) {
  for (const file of files) {
    ...
    if (!new RegExp(`\\b${importMatch[1]}\\b`).test(allSpecimensBlock)) {
      errors.push(
