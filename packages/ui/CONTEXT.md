...
packages/ui/
  src/
    ...
    types/ui.types.ts UiTone / UiVariant / UiSize / UiFontSize — the exported prop types
...
scale used by `Heading`/`Text`/`Display`/`Eyebrow` — it maps to font-size steps,
so it carries the extreme ends (`2xs`, `2xl`) plus a `default` per-level
fallback. They are separate contracts: never merge or alias them.

Components narrow with `Exclude<>`/`Extract<>` (`BadgeVariant = Exclude<UiVariant,
'ghost'>`, `ButtonVariant = Exclude<UiVariant, 'soft'>`, `EyebrowFontSize =
Extract<UiFontSize, 'small' | 'medium' | 'large' | 'xl'>`). Always import from
`ui.types.ts`; never re-declare locally. There are no ambient global UI types —
every shared type is exported from this module.
...
- Solid fills always use `text-tone-<tone>-contrast`, never a literal
  white/black. Accent fills pair with `--ui-accent-contrast` the same way.
- `--ui-overlay` is the contract token for modal scrims (default
  `rgb(0 0 0 / 0.7)`, overridable per mode in `theme.json`) — reach for it
  rather than a hardcoded black when styling overlay surfaces.
...
12 palette colours — 3- or 6-digit hex only, no alpha — 3 fonts, 3 radii,
3 shadows, border, motion, display case —
...
6. Add `<name>.specimen.tsx` using `defineSpecimen<Props>()` and register it in
   ...
   Two suites then run the whole specimen matrix: `specimens.render.test.tsx`
   under jsdom, and `specimens.ssr.test.tsx` under a real node environment —
   the latter is what catches a component touching `window`/`document`/
   `matchMedia` during render instead of in an effect.
...
- Keep `COMPONENTS.md` and the specimen registry in sync in the same change;
  ...
  `variants` (the in-package render and SSR tests fail otherwise).
