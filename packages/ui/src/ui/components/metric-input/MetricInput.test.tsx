...
const getInput = () => screen.getByRole('spinbutton', { name: 'Weight' }) as HTMLInputElement;
...
describe('MetricInput accessibility & passthrough', () => {
  ...
  it('merges className onto the root and forwards id', () => {
    ...
    expect(input.parentElement?.className).toContain('custom-root');
    expect(input.parentElement?.className).toContain('flex');
  });
