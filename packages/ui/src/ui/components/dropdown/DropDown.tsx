...
export interface DropDownProps {
   ...
   */
  searchable?: boolean;
  /** Controlled selected value(s). Pair with `onChange` for controlled usage. */
  value?: string | string[];
...
export const DropDown = ({
  ...
  // The filter box borrows the trigger's own name, so a page rendering several
  // searchable dropdowns doesn't announce the same generic label for each one.
  ...
  // where an effect would leave it generically named for a frame.
  const handleOpenChange = (next: boolean) => {
    if (next && searchable && ariaLabelledBy) {
  ...
  const visibleChips = selectedValues.slice(0, maxCount);
