...
describe('form.TextField', () => {
  ...
  describe('native picker types', () => {
    type Values = { schedule: string };

    const PickerDemo = ({ type }: { type: string }) => {
      const form = useAppForm({ defaultValues: { schedule: '' } as Values });

      return (
        <Form form={form}>
          <form.TextField label="Starts" name="schedule" type={type} />
        </Form>
      );
    };

    const renderPicker = async (type: string) => {
      const rendered = await renderReact(<PickerDemo type={type} />);
      const input = requireElement(
        rendered.container.querySelector<HTMLInputElement>('input'),
        `${type} input`,
      );
    ...
    it.each(['date', 'time', 'datetime-local', 'month', 'week'])(
      'opens the native picker when a %s field is focused',
      async (type) => {
    ...
    it.each(['text', 'email', 'password', 'number'])(
      'does not open a picker for a %s field',
      async (type) => {
        const showPicker = stubShowPicker();
        const { input, rendered } = await renderPicker(type);
    ...
    it('opens the picker once for the click that moves focus into the field', async () => {
      const showPicker = stubShowPicker();
    ...
    it('does not call showPicker on a disabled field', async () => {
      ...
      const Demo = () => {
        ...
        return (
          <Form form={form}>
            <form.TextField disabled label="Starts" name="schedule" type="date" />
          </Form>
        );
      };
