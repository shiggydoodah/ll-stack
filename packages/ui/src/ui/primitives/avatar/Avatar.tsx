 ...
 */
export interface AvatarProps extends Omit<ComponentPropsWithoutRef<'span'>, 'color'> {
  ...
  /** Accessible name for the avatar; required when `src` is set. */
  alt?: string;
  ...
  return (
    <span className={cn(avatarBaseClass, avatarSizeClasses[size], className)} {...props}>
      {asChild ? (
        ...
        <span aria-label={alt} className={cn(avatarInitialsClass, ring && avatarRingClass)}>
          {initials}
        </span>
      )}
      {online && (
