...
describe('formatRelativeTime', () => {
  ...
  it('falls back to an absolute MM/DD/YYYY date at 48 hours and beyond', () => {
    // 48h before 10 Jun 2026 12:00 local => 8 Jun 2026.
    expect(formatRelativeTime(ago(48 * HOUR_MS), now)).toBe('06/08/2026');
  });

  it('zero-pads single-digit months and days', () => {
    const value = new Date(2026, 5, 8, 9, 30, 0); // 8 Jun 2026
    const reference = new Date(2026, 5, 20, 9, 30, 0); // 12 days later
    expect(formatRelativeTime(value, reference)).toBe('06/08/2026');
  });
  ...
  it('leaves two-digit months and days unpadded-but-correct', () => {
    ...
    expect(formatRelativeTime(value, reference)).toBe('11/25/2026');
  });
...
describe('formatRelativeTimeCompact', () => {
  ...
  it('falls back to an absolute MM/DD/YYYY date at 7 days and beyond', () => {
    // 7d before 10 Jun 2026 12:00 local => 3 Jun 2026.
    expect(formatRelativeTimeCompact(ago(7 * DAY_MS), now)).toBe('06/03/2026');
  });
