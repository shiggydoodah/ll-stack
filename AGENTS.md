...
- Review agents must ignore changed files under `apps/frontend/app/dev/**`. This path is reserved and does not exist yet — `DEV_MODE` and `apps/frontend/config/dev-mode.ts` are the switch that will gate it. Once it exists it is local-only experimental and dev-test surface; do not review, summarize, comment on, or request production hardening, strict coding standards, UI polish, test coverage, validation, local architecture, or development-only implementation fixes for files that live there.
...
`.github/workflows/ci.yml` runs the same ladder plus `pnpm format:check` on every push to `main` and every pull request. It is a backstop, not a substitute: CI runs no command you cannot run locally, so a red build means the local run was skipped, not that CI is different. `pnpm test:e2e` is NOT in CI — the `e2e` job is commented out deliberately (see `.github/CONTEXT.md`), so Playwright is a local-only gate.
