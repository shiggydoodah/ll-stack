...
| `MemberSidebar.tsx`   | `'use client'`. The `(members)` shell navigation, mounted by that group's layout. Client-side for one reason: the active item comes from `usePathname`, which a layout cannot pass down.             |
| `AppToaster.tsx`      | Toast host. Not mounted anywhere yet — `notify()` has no outlet until a layout renders this.                                                                                                         |
