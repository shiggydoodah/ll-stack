...
- **App Router** (`app/`) — route groups `(public)/(guest)` and `(members)`,
  plus `api/client-logs`. Server Components by default. The `(members)` layout
  guards and then mounts the shared shell, so `/dashboard` and `/account` render
  inside one sidebar. See `app/CONTEXT.md`.
- **`proxy.ts`** — the Next proxy (middleware equivalent). Mints the per-request
  ...
  redirects guest↔member routes on cookie presence, rolls the session-binding
  cookies (the idle timeout), and drives session-token rotation. It is the only
  place in this app that runs on every member request and can still write
  cookies, which is why the rotation call lives there. → `lib/auth/CONTEXT.md`
- **`lib/`** — all non-route server code: `gateway/` (the only way to call the
  backend), `actions/` (`actionWrapper`), `authentication/`, `auth/` (binding
  token, idle timeout, rotation), `logging/`, `errors/`, `cache/`, `routes.ts`,
  `constants/`.
- **`components/`** — app-shared components, including `MemberSidebar` (the
  `(members)` shell). Route-specific ones live in that route's `_components/`.
...
guest/member redirects, binding cookies, rotation when due) → layout guard
(`validateSession()`) → page → `lib/gateway/*` → backend (with `x-api-secret`,
session cookie, and `x-request-id`/`x-session-id`).
...
`llstack_session` locally, mint the binding cookie pair (`setBindingCookies`),
rotate the `llstack_sid` log session id, and redirect to `/dashboard`. Logout is
a **GET route handler** (`/logout`) so any redirect can trigger it; it 403s every
cross-site request that is not a top-level navigation carrying a token this
server minted for that browser's session cookie (`lib/auth/logout-token.ts`),
then revokes best-effort and always clears every auth cookie. A lapsed binding is the idle timeout and takes the same route, as
does a session the backend has refused — a revoked family under the
rotation-reuse alarm — see `lib/auth/CONTEXT.md`.

**Logging:** browser (`clientLogger`) → `POST /api/client-logs` (rate-limited —
`lib/logging/client-log-rate-limit.ts`) → shared sink;
...
- Env: `BACKEND_INTERNAL_URL`, `BACKEND_API_SECRET`, `BINDING_SECRET`,
  ...
  `TRUST_PROXY`, `CLIENT_LOG_INGEST_ENABLED`, `CLIENT_LOG_ALLOWED_ORIGIN`,
  `CLIENT_LOG_RATE_LIMIT_PER_MINUTE`,
  `CLIENT_LOG_RATE_LIMIT_SHARED_PER_MINUTE`,
  `CLIENT_LOG_RECORD_LIMIT_PER_MINUTE`,
  `CLIENT_LOG_RECORD_LIMIT_SHARED_PER_MINUTE`, `FRONTEND_INSTANCE_COUNT`.
