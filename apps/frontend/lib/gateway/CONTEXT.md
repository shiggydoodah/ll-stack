...
- One module per backend domain, each exporting app-shaped functions:
  - `auth.ts` — `register`, `login` (both `withAuth: false`), `logout`,
    `listSessions`, `revokeAllSessions(keepCurrent)`, and
    `rotateSession(sessionToken)`, which takes the session as an argument because
    its only caller is `proxy.ts` and middleware has no `next/headers`. It is the
    only call here with an `AbortSignal`: running before the render means a hung
    backend would hold the navigation open, so it is capped at
    `ROTATE_SESSION_TIMEOUT_MS` and the abort reads as a 503.
    `revokeAllSessions` takes `keepCurrent` as a required argument rather than
    defaulting it: the backend revokes everything when it is absent, so a caller
    that forgets it signs the member out of the page they are standing on.
