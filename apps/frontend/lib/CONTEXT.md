...
| `auth/`           | The session **binding** token (HMAC over the session token, a strict + lax cookie pair), the **idle timeout**, and session-token **rotation**. Separate from `authentication/` on purpose. → `auth/CONTEXT.md` |
...
- There are two session clocks. The backend's `AUTH_SESSION_TTL_SECONDS` (7
  days) is the ceiling; `AUTH_IDLE_TIMEOUT_SECONDS` (8 hours, `lib/auth/`) is
  the idle window inside it and the one users actually meet.
