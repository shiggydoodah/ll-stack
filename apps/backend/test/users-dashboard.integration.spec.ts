...
describe('Users + dashboard endpoints (integration)', () => {
  ...
  describe('GET /users/me', () => {
    ...
    it('rejects a missing session cookie with 401 SESSION_INVALID', async () => {
      const response = await get('/users/me').expect(401);
      expect(response.body.error).toBe('SESSION_INVALID');
    });

    it('rejects a garbage session cookie with 401 SESSION_INVALID and clears it', async () => {
      // The code matters, not just the status: the frontend signs a browser out
      // on 401 + SESSION_INVALID alone, so a bare 401 here would leave dead
      // cookies unrecoverable client-side.
