...
export class SessionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Both refusals carry `error: 'SESSION_INVALID'`, the same envelope the
    // rotate route answers with. The frontend signs a browser out on that code
    // alone — a bare 401 also covers a wrong `x-api-secret`, and reading it as
    // a dead session turns a misconfigured secret into a mass sign-out.
    const token = this.sessionCookies.read(req);
    if (token === null) {
      throw new UnauthorizedException({ message: 'No session cookie', error: 'SESSION_INVALID' });
    }
