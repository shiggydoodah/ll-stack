...
`AUTH_SESSION_ROTATE_AFTER_SECONDS`, `AUTH_SESSION_ROTATION_GRACE_SECONDS`,
...
- `AUTH_SESSION_ROTATE_AFTER_SECONDS` < `AUTH_SESSION_TTL_SECONDS`, and
  `AUTH_SESSION_ROTATION_GRACE_SECONDS` < the rotation interval (any
  environment). The first would mean the token is never re-issued; the second
  would keep every superseded token usable and disable reuse detection.
