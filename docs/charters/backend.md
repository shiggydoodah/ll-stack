...
The verify ladder runs lint → build → typecheck → test against Node 22 + Postgres 16. Tests that pass locally but need an unmigrated schema will fail against a fresh database — run migrations against the test DB (`pnpm migrate`) before integration work.
...
Why: these are exactly the questions the PR will be reviewed against — `docs/charters/pr-writting.md` § Backend Change-Impact Standard and § Backend Verification Matrix. A plan that answers them up front turns review findings into design decisions.
