...
- Validate that session cookies remain HttpOnly and server-managed — written via `next/headers` in server-only code, or on the `NextResponse` in `proxy.ts`, which is the only other writer and the one that handles token rotation.
