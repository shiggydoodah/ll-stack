...
- `apps/backend/src/auth` - register/login/logout, session issue + rotation + reuse detection, the member's own session list and bulk revoke, `SessionGuard`, named throttlers, session prune. Context: `apps/backend/src/auth/CONTEXT.md`
...
- `apps/frontend/components` - app-shared components (`LoggingProvider`, `ErrorScreen`, `NotFoundScreen`, `ModeToggle`, `MemberSidebar`, `AppToaster`). Context: `apps/frontend/components/CONTEXT.md`
...
- `apps/frontend/lib/auth` - session-**binding** HMAC token, its strict + lax cookie pair, the idle timeout (`AUTH_IDLE_TIMEOUT_SECONDS`), the session-rotation call `proxy.ts` makes, and the `/logout` redirect token. Not the session cookie. Context: `apps/frontend/lib/auth/CONTEXT.md`
